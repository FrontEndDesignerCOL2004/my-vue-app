...
export const obtenerPlanPorId = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
export const obtenerPlanesPorTipo = async (req: Request, res: Response) => {
  try {
    ...
    const planes = await Plan.find({ type: tipo }); //En el modelo la propiedad es 'Type'
    res.json(planes);
  } catch (err) {
...
export const crearPlan = async (req: Request, res: Response) => {
  try {
    ...
    const nuevoPlan = new Plan({ type, name, benefits, price });
...
export const actualizarPlan = async (req: Request, res: Response) => {
  try {
    ...
    if (error.name === "ValidationError") {
      const mensajes = Object.values(error).map((err: any) => err.message);
